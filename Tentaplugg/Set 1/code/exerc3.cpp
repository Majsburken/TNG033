...
std::vector<int> merge_seq(const std::vector<int>& S1, const std::vector<int>& S2) {
    ...
    //Keeps track of S1 and S2:s indices
    int count1 = 0;
    int count2 = 0;

    //Keeps going until we are at the end of either S1 or S2
    while (count1 < std::ssize(S1) && count2 < std::ssize(S2)) {

        //Adds S1[count1] to S3 and increments S1:s counter
        if (S1[count1] < S2[count2]) {
            S3.push_back(S1[count1]);
            count1++;
        }
        //Adds S2[count2] to S3 and increments S2:s counter
        else if (S2[count2] < S1[count1]) {
            S3.push_back(S2[count2]);
            count2++;
        }
        //S1[count1] or S2[count2] does not matter which, as they have teh same value
        //Adds S1[count1] to S3 and increments S1 and S2:s counters
    ...
    //If end of S2 but not end of S1
    //Add remaining (in S1) values to S3
    while (count1 < std::ssize(S1)) {
        S3.push_back(S1[count1]);
        count1++;
    }

    //If end of S1 but not end of S2
    //Add remaining (in S2) values to S3
